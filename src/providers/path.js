...
export function PathProvider({ children, location: { pathname } }) {
  ...
  if (ref.current[1] !== path) {
    const withoutPrefix =
      pathPrefix !== slash && path.startsWith(pathPrefix) ? path.replace(pathPrefix, slash) : path
    ref.current = [withoutPrefix, path]
  }

  return <PathContext.Provider value={ref.current}>{children}</PathContext.Provider>
}
